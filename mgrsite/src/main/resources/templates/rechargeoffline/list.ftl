	...
	<div class="container">
		...
		<div class="row">
			...
			<div class="col-sm-9">
				...
				<form id="searchForm" class="form-inline" method="post" action="/rechargeOffline">
					    ...
					    <select class="form-control" name="bankInfoId">
					    	...
					    	<#list banks as bank>
	    						<option value="${bank.id}">
									<script>
										var str="("+SITE_BANK_TYPE_NAME_MAP["${bank.bankName}"]+")${bank.accountNumber}${bank.bankForkName}";
										document.write(str);
									</script>
								</option>
	    					</#list>
					...
					<table class="table">
						...
						<#list pageResult.listData as info>
							<tr>
								...
								<td>${info.tradeTime?string('yyyy-MM-dd')}</td>
								<td>${info.amount}</td>
								<td>
									<script>
										var str="("+SITE_BANK_TYPE_NAME_MAP["${info.bankInfo.bankName}"]+")${info.bankInfo.accountNumber}${info.bankInfo.bankForkName}";
										document.write(str);
									</script>
								</td>
		  ...
		  <div class="modal-dialog modal-lg" role="document">
		    <div class="modal-content">
		      <div class="modal-body">
		      	<form class="form-horizontal" id="editform" method="post" action="/rechargeOffline_audit">
					<fieldset>
						<div id="legend" class="">
