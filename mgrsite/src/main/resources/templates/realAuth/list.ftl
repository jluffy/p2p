...
<script type="text/javascript">
	$(function() {
        ...
        $(".auditClass").click(function(){
            ...
            $("#sex").text(data.sex);
            $("#birthDate").text(data.bornDate);
            $("#address").text(data.address);
            $("#image1").prop("src",data.image1);
            $("#image2").prop("src",data.image2);
            $("#myModal").modal("show");
        });
        //审核
			...
			<div class="col-sm-9">
				...
				<form id="searchForm" class="form-inline" method="post" action="/realAuth">
					...
					<div class="form-group">
						...
						<script type="text/javascript">
							$("[name=state] option[value='${(qo.state)!''}']").prop("selected",true);
						</script>
					</div>
					<div class="form-group">
					    <label>申请时间</label>
					    <input class="form-control" type="text" name="beginDate" id="beginDate" value='${(qo.beginTime?string("yyyy-MM-dd"))!""}' />到
					    <input class="form-control" type="text" name="endDate" id="endDate" value='${(qo.endTime?string("yyyy-MM-dd"))!""}'/>
					</div>
					<div class="form-group">
